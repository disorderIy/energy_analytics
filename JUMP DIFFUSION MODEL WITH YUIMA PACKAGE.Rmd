...
\section{Methodology (Describing the dynamics of the Jump Diffusion model)}
...
\begin{enumerate}
\item P(X_{0}=0)=1
\item Stationary increments: the distribution $X_{t+s}-X_{t}$ over the interval $[t, t+s]$ is independent of time.
\item Independent increments: for every increasing sequence of time $t{0},...,t_{n}$, the random variables $X_{t_{0}}, X_{t_{1}}-X_{t_{0}},...,X_{t_{n}}-X_{t_{n-1}}$ are independent.
\item Stochastic continuity so that discontinuity occurs at random times i.e. $\forall \epsilon > 0, \lim_{h \rightarrow 0} P(|X_{t+s}-X_{t}| \geq \epsilon)$
\item The paths $X_{t}$ are \textit(Cadlag) paths i.e. almost surely right continuous with left limits.
...
\section{Discussion on Parameters Calibration using Yuima package in R}
